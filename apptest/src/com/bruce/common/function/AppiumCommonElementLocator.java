...
public class AppiumCommonElementLocator {

	AndroidDriver driver;

	private static int WAIT_TIME = 3; // 默认的等待控件时间

	public AppiumCommonElementLocator(AndroidDriver androidDriver) {
	...
	public AndroidElement waitAuto(final By by, int time) {
		try {
			return new AndroidDriverWait(driver, time).until(new ExpectedCondition<AndroidElement>() {
				@Override
